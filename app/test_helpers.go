...
func setup(t testing.TB, withGenesis bool, invCheckPeriod uint, opts ...wasm.Option) (*TokenApp, GenesisState) {
	nodeHome := t.TempDir()
	snapshotDir := filepath.Join(nodeHome, "data", "snapshots")
	snapshotDB, err := sdk.NewLevelDB("metadata", snapshotDir) //nolint:staticcheck // TODO: fixme
	require.NoError(t, err)
	snapshotStore, err := snapshots.NewStore(snapshotDB, snapshotDir)
	require.NoError(t, err)
	// baseAppOpts := []func(*bam.BaseApp){bam.SetSnapshotStore(snapshotStore), bam.SetSnapshotKeepRecent(2)}
	snapshotOps := types.SnapshotOptions{
		Interval:   2,
		KeepRecent: 0,
	}
	baseAppOpts := []func(*bam.BaseApp){bam.SetSnapshot(snapshotStore, snapshotOps)}
	db := dbm.NewMemDB()
...
func Setup(isCheckTx bool, opts ...wasm.Option) *TokenApp {
	db := dbm.NewMemDB()
...
func SetupWithGenesisValSet(t *testing.T, valSet *tmtypes.ValidatorSet, genAccs []authtypes.GenesisAccount, opts []wasm.Option, balances ...banktypes.Balance) *TokenApp {
...
func AddTestAddrsFromPubKeys(app *TokenApp, ctx sdk.Context, pubKeys []cryptotypes.PubKey, accAmt math.Int) {
...
func AddTestAddrs(app *TokenApp, ctx sdk.Context, accNum int, accAmt math.Int) []sdk.AccAddress {
...
func addTestAddrs(app *TokenApp, ctx sdk.Context, accNum int, accAmt math.Int, strategy GenerateAccountStrategy) []sdk.AccAddress {
...
func CheckBalance(t *testing.T, app *TokenApp, addr sdk.AccAddress, balances sdk.Coins) {
...
func SignCheckDeliver(
	...
	gasInfo := sdk.GasInfo{GasWanted: uint64(r.GasWanted), GasUsed: uint64(r.GasUsed)}
	resData := sdk.Result{
		Data:         r.Data,
		Log:          r.Log,
		Events:       r.Events,
